...
class Lkpp{
	...
	function addArsiplkpp($jenis_arsip,$nama,$ruang,$rak,$baris,$box,$keyword,$arsip_id,$tanggal,$uraian,$attachment){
		...
		$query2=$this->db1->prepare("INSERT INTO lkpp_detail(arsip_id,tanggal,uraian,attachment) VALUES (?,?,?,?)");
	...
	function updateLkpp($id,$nama,$ruang,$rak,$baris,$box,$keyword,$arsip_id,$tanggal,$uraian,$attachment){
		$this->db1->beginTransaction();
	...
	function deleteLkpp($id){
		$query=$this->db1->query("DELETE arsip,lkpp_detail FROM arsip,lkpp_detail WHERE lkpp_detail.arsip_id = arsip.arsip_id  AND arsip.arsip_id='$id'");
		$jml_data=$query->rowCount();
		return $jml_data;
	}			
}	
